...
def reverse_Rank(G, Ep, T, min_size):
    '''
     Input:
...
if __name__ == "__main__":
    ...
    reverse_filename = FOLDER + REVERSE_FOLDER + "/%s_%s_%s_%s_option2.txt" % (
    REVERSE_FOLDER, ALGO_NAME, dataset, model)
    ...
    for T in range(100, 2100, 100):
        ...
        print
        'Preprocessing to find minimal size of CC...'
        ...
        'Min size:', min_size
        print
        'Finished preprocessing in %s sec' % (time.time() - time2preprocess)

        print
        'Start mapping...'
        time2map = time.time()
        result = pool.map(mapReverseCCWP,
                          ((G, Ep, T, min_size) for i in range(R)))  # result is [(scores1, L1), (scores2, L2), ...]
        print
        'Finished mapping in %s sec' % (time.time() - time2map)

        print
        'Start reducing scores...'
        ...
        for (Sc, L) in result:
            ...
            for (node, score) in Sc.iteritems():
                scores[node] += score
        print
        'Finished reducing in %s sec' % (time.time() - time2reduce)
        print
        'avgL', avgL
        print
        'minL', minL
        ...
        print
        '|S|: %s --> %s nodes | %s sec' % (len(S), coverage, time2coverage)
        ...
        while Low + 1 != High:
            ...
            time2coverage = time.time() - time2Ts
            print
            '|S|: %s --> %s nodes | %s sec' % (len(lastS), coverage, time2coverage)
        ...
        # print finalS
        print
        "Number of binary steps:", len(Coverages) - 1
        with open(steps_filename, 'a+') as fp:
            print >> fp, T, len(Coverages) - 1
        print
        'Necessary %s initial nodes to target %s nodes in graph G' % (len(finalS), T)
        with open(reverse_filename, 'a+') as fp:
            print >> fp, T, High

        print
        'Finished seed minimization for T = %s in %s sec' % (T, time.time() - time2select)
        print
        '----------------------------------------------'
